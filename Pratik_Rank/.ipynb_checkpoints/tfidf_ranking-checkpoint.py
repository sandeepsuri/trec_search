...
should be modeled using a log function. Following BM25+, they add (d) to ensure there is a sufficient gap between 
...
class TFIDF:
    ...
    def average_document_length(self):
        ...
        """
        if TFIDF.useCache:
            return TFIDF.average_doc_length
        else:
            summ = 0
    ...
    def modified_idf_calculation(self, query_word):
        """
        Modified IDF calculation for T d IDF
    ...
    def no_of_documents_containing_a_word(self, query_word):
        ...
        """
        if TFIDF.useCache:
            if query_word in TFIDF.no_of_docs_dict:
                return float(TFIDF.no_of_docs_dict[query_word])
            else:
                return 0
        else:
    ...
    def score(self, query, document_id):
        ...
        for key, value in query[2].items():
            ...
            tf = self.word_frequency_of_word_in_document(key, document_id)
            
            #IDF(t) = 
            #log_e(Total number of documents / Number of documents with term t in it)
            idf = self.modified_idf_calculation(key)

            tf_idf = tf * idf
        tup = (query, document_id, tf_idf)
        return tup
